...
# Let's find exactly the class Particle { ... } draw() { ... } }
# It starts at `class Particle {` and ends at the FIRST `}` after `draw() {`

start_marker = "class Particle {"

start_idx = content.find(start_marker)

# Let's find "for (let i = 0;"
end_idx = content.find("for (let i = 0;", start_idx)
...
content = content[:start_idx] + new_particle_class + content[end_idx:]

with open('fastapi_app/static/main.js', 'w', encoding='utf-8') as f:
    f.write(content)

print("Particle patched successfully")
