...
target = """            plates_count = 'Metrik Ayarlanmad?'
            if metric_table:
                try:
                    count_row = await conn.fetchrow(f'SELECT count(*) as count FROM "{metric_table}"')
                    if count_row:
                        plates_count = f"{count_row['count']} Kay?t ({metric_table})"
                except Exception:
                    plates_count = f"Tablo Bulunamad? ({metric_table})""""

# Or search without exact Turkish chars
...
# Also update lifespan in main.py
